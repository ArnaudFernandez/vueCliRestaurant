...
function putRequest(form) {
    ...
    .then(function(responseJSON) {responseJSON.json()
            .then(function(res) {
                // Maintenant res est un vrai objet JavaScript
...
function afficheReponseDELETE(reponse) {
    let div = document.querySelector("#responseDELETE");
    div.innerHTML = reponse.msg;
}
...
function supprimerRestaurant(url, jsObject) {
    var id = jsObject.restaurant._id;
    console.log("on supprime le restaurant id=" + id);

    envoieRequeteFetchDelete(id);
}

function modifierRestaurant(noLigne,restaurant) {
